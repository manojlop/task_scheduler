...
void Scheduler::notifyDependents(TaskID taskId){
        ...
        if(downwardDependencies_.count(currFailed)){
          for(auto dependent_id : downwardDependencies_.at(currFailed)){
            ...
            if(dep_it != tasks_.end()){
              ...
              safe_print(("Task with id: " + std::to_string(dependent_id) + " couldn't be found in tasks map"), "Scheduler", t_Verbosity::ERROR);
            }
          }
        }
      }
  ...
  if (completed_task_ptr->getState() != t_TaskState::COMPLETED) {
    ...
    safe_print(("Task with id: " + std::to_string(taskId) + " wasn't COMPLETED nor FINISHED, but notify dependents was called"), "Scheduler", t_Verbosity::ERROR);
    return;
  }
  ...
  if(downwardDependencies_.count(taskId)){
    for(auto dependent_id : downwardDependencies_.at(taskId)){
      ...
      if(dep_ptr->getState() == t_TaskState::PENDING){
        ...
        safe_print(("Task with id: " + std::to_string(dep_ptr->id_) + " is already in state: " + taskStateName[int(dep_ptr->getState())] + " but it depends on task: " + std::to_string(taskId) + " which has just COMPLETED"), "Scheduler", t_Verbosity::ERROR);
      }
    }
  }
}
...
TaskID Scheduler::addTask(std::function<void()> func, const std::vector<TaskID>& dependencies){
  safe_print(("Adding new task"), "Scheduler", t_Verbosity::DEBUG);
  ...
  for(auto depends_on : dependencies){
    if(tasks_.count(depends_on) == 0){
      safe_print(("Tried to add dependency for id: " + std::to_string(currentId) + " to " + std::to_string(depends_on) + " which is not in already created tasks"), "Scheduler", t_Verbosity::ERROR);
      return -2;
    }
  ...
  try{
    ...
    safe_print("Unable to create unique pointer for thread", "Scheduler", t_Verbosity::ERROR);
    return -1;
  }
  ...
  for(auto dep : dependencies){
    ...
    if(tasks_[dep]->getState() == t_TaskState::COMPLETED) {
      safe_print(("Task: " + std::to_string(currentId) + "  depends on task: " + std::to_string(dep) + " which is already COMPLETED, so counter is decremented"), "Scheduler", t_Verbosity::INFO);
      tasks_[currentId]->decrement_unmet_dependencies();
    }
    // FAILURE is propagated
    else if (tasks_[dep]->getState() == t_TaskState::FAILED) {
      safe_print(("Task: " + std::to_string(currentId) + "  depends on task: " + std::to_string(dep) + " which has FAILED, so its also FAILS"), "Scheduler", t_Verbosity::INFO);
  ...
  if(tasks_[currentId]->unmetCount_.load() == 0){
    ...
    safe_print(("Task with id: " + std::to_string(currentId) + " doesn't have any more unmet dependencies, so it is READY"), "Scheduler", t_Verbosity::INFO);
    ...
    workAvailable_.notify_one();
     ...
     // Ensure initial state is PENDING if it wasn't set by default
      ...
      safe_print(("Task with id: " + std::to_string(currentId) + " is PENDING"), "Scheduler", t_Verbosity::INFO);
  }
  return currentId;
}
...
void Scheduler::start(){
  if (state_ == t_SchedulerState::STARTED) {
    safe_print("Scheduler already started", "Scheduler", t_Verbosity::WARNING);
    return;
  }
  if(state_ == t_SchedulerState::CREATED) {
    ...
    for(size_t i = 0; i < workers_.size(); i++){
      ...
      try {
        ...
        safe_print(("Unable to start thread: " + std::to_string(i)), "Scheduler", t_Verbosity::ERROR);
        return;
      }
    }
  state_ = STARTED;
...
void Scheduler::stop(t_StopWay wayToStop){
  if(state_ != t_SchedulerState::STOPPED) {
    ...
    safe_print("Lock acquired to stop the scheduler", "Scheduler", t_Verbosity::DEBUG);
    ...
    for(auto taskId : readyTasks_) {
      ...
      safe_print(("Scheduler is being stopped, so task: " + std::to_string(taskId) + " which was READY is being CANCELLED"), "Scheduler", t_Verbosity::DEBUG);
    ...
    lck.unlock();
    safe_print("Lock released to stop the scheduler", "Scheduler", t_Verbosity::DEBUG);
    ...
    for(int i = 0; i < int(workerThreads_.size()); i++){
      ...
      if (thread.joinable()) {
        ...
        safe_print(("Worker: " + std::to_string(i) + " is still busy, so we wait for it"), "Scheduler", t_Verbosity::NONE);
        thread.join();
        safe_print(("Worker: " + std::to_string(i) + " has finished work"), "Scheduler", t_Verbosity::NONE);
      }
    }
...
bool Scheduler::waitTasksToEnd(){
  if(state_ == t_SchedulerState::STARTED){
    std::unique_lock<std::mutex> lck(mtx_);
    safe_print("Waiting for all tasks to end", "Scheduler", t_Verbosity::INFO);
...
void Scheduler::Worker::run(){
  safe_print(("Started work"), ("Worker: " + std::to_string(this->id_)), t_Verbosity::INFO);
  while(true){
    int currTaskID = -1;
    bool stopReq = false;
    ...
    if(stopReq){
      // Todo : implement end of life thread -> One thread to run to "cleanup" everything (or wait for all processes to finish?)
      safe_print(("Stop was requested from scheduler"), ("Worker: " + std::to_string(this->id_)), t_Verbosity::NONE);
      lck.unlock();
      break;
    }
    
    safe_print(("Its trying to find task to work on"), ("Worker: " + std::to_string(this->id_)), t_Verbosity::DEBUG);
    ...
    if (it == scheduler_.tasks_.end()) {
      safe_print(("Task with id: " + std::to_string(currTaskID) + " couldn't be found in tasks map"), ("Worker: " + std::to_string(this->id_)), t_Verbosity::ERROR);
      lck.unlock(); 
      continue;
    }
    task_ptr = it->second; // Get the shared_ptr
    safe_print(("Is responsible for task: " + std::to_string(currTaskID)), ("Worker: " + std::to_string(this->id_)), t_Verbosity::INFO);
    ...
    if(task_ptr){
      try{
        safe_print(("Is processing task: " + std::to_string(currTaskID)), ("Worker: " + std::to_string(this->id_)), t_Verbosity::INFO);
        task_ptr->run();
        safe_print(("Has COMPLETED to process task: " + std::to_string(currTaskID)), ("Worker: " + std::to_string(this->id_)), t_Verbosity::INFO);
      } catch (...){
        safe_print(("Has FAILED to process task: " + std::to_string(currTaskID)), ("Worker: " + std::to_string(this->id_)), t_Verbosity::INFO);
        succesfullTask = false;
      }
    }

    lck.lock();
    safe_print(("Acquired end of task lock: " + std::to_string(currTaskID)), ("Worker: " + std::to_string(this->id_)), t_Verbosity::DEBUG);
    if(succesfullTask) {
      safe_print(("Has processed task: " + std::to_string(currTaskID) + " and it is COMPLETED"), ("Worker: " + std::to_string(this->id_)), t_Verbosity::INFO);
      task_ptr->setState(t_TaskState::COMPLETED);
      scheduler_.completedTasks_.fetch_add(1);
    } else { 
      safe_print(("Has processed task: " + std::to_string(currTaskID) + " and it is FAILED"), ("Worker: " + std::to_string(this->id_)), t_Verbosity::INFO);
    ...
    lck.unlock();
    safe_print( "Released end of task lock", ("Worker: " + std::to_string(this->id_)), t_Verbosity::DEBUG);

    safe_print( "No more work to do", ("Worker: " + std::to_string(this->id_)), t_Verbosity::DEBUG);
    task_ptr = nullptr;
  } 
}
...
void Scheduler::printTaskCollection(t_Verbosity verb){
  ...
  for(auto task : tasks_){
    ...
    std::string str = "Task: " + std::to_string(task.first) + " is " + taskStateName[int(task.second->getState())] + " unmet count is : " + std::to_string(task.second->unmetCount_) + " and it depends on following tasks: " + dependencies;
